...
local DarkHub = game:GetObjects("rbxassetid://10804731440")[1]

DarkHub.Enabled = false

if gethui then
	DarkHub.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(DarkHub)
	DarkHub.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	DarkHub.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	DarkHub.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == DarkHub.Name and Interface ~= DarkHub then
			Interface.Enabled = false
			Interface.Name = "DarkHub-Old"
		end
	end
else
...
if DarkHub.AbsoluteSize.X < minSize.X and DarkHub.AbsoluteSize.Y < minSize.Y then
...
function ChangeTheme(ThemeName)
	SelectedTheme = DarkHubLibrary.Theme[ThemeName]
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if DarkHubLibrary.Flags[FlagName] then
			...
			DarkHubLibrary:Notify({Title = "Flag Error", Content = "DarkHub was unable to find '"..FlagName.. "'' in the current script. Check docs.sirius.menu for help."})
		end
	end
end
			...
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						warn('Check docs.sirius.menu for help with DarkHub specific development.')
					end
...
function Hide(notify: boolean?)
	Debounce = true
	if notify then
		DarkHubLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K.", Duration = 7})
	end
...
function DarkHubLibrary:CreateWindow(Settings)
	...
	pcall(function()
		...
		if not isfolder(DarkHubFolder.."/".."Configuration Folders") then
	...
	if (Settings.KeySystem) then
		...
		if isfile(DarkHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(DarkHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
...
function DarkHubLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function DarkHubLibrary:IsVisible(): boolean
	return not Hidden
end
...
task.delay(3.5, DarkHubLibrary.LoadConfiguration, DarkHubLibrary)
