...
function DarkHubLibrary:CreateWindow(Settings)
    ...
    local pastebinURL = "https://pastebin.com/raw/U7CfzUQH"  -- Replace with your Pastebin link
    local Success, Response = pcall(function()
        local keysText = game:HttpGet(pastebinURL)
        Settings.KeySettings.Key = {}
        for key in keysText:gmatch("%S+") do
            table.insert(Settings.KeySettings.Key, key)
        end
    end)
    if not Success then
        print("Error fetching keys from Pastebin: " .. tostring(Response))
    end
end
